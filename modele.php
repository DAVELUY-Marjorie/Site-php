...
function _getEvent($conn, $tri, $titre, $date=null, $pos=null)
{
  ...
  $query = "SELECT ID, Prenom, Nom, Description, Date, IdOrga, Lieu, Titre, Type, NombreParticipe FROM Event JOIN user WHERE Event.IdOrga = user.login";
	...
	if ($tri=='NombreParticipe') {
		$query = $query." ORDER BY Visiteur DESC, Adherent DESC, Gestionnaire DESC ";
	}else{
...
function supprEvent($idEvent, $co){
	//supprimer tt les participations
	// faire moins 1 à tt les "événénement" dans user
	//PUIS suppr l'évent
...
function _inscrire($co, $log, $mdp, $mail, $role, $prenom, $nom, $image){
	// check que le login n'existe pas déjà
